...
class Model {
  ...
  static Future<Model?> load(String path, Shader shader, [double scale = 1.0, DefaultTextures? defaultTextures]) async {
    ...
    */
    ...
    for (int i = 0; i < scene.mNumMeshes; i++) {
      ...
      final indices = List.generate(assimpMesh.mNumFaces, (index) => assimpMesh.mFaces[0]).expand((face) => List.generate(face.mNumIndices, (index) => face.mIndices[index]));

      meshes.add(Mesh(Mesh.makeVerticesArrayFromComponents(positions.length, positions, normals, uvs)));
    }
