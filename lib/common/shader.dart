...
class Shader {
  ...
  Shader(String vert, String frag) {
    ...
    malloc.free(status);
    if (_status != GL_TRUE) throw Exception('An error occurred while compiling vertex shader ($vertexShader):\n\n{GL.GetShaderInfoLog(vertexShader)}');

    final fragmentShader = gl.glCreateShader(GL_FRAGMENT_SHADER);
    gl.glShaderSource(fragmentShader, 1, frag.toNativeUtf8().cast(), nullptr);
    gl.glCompileShader(fragmentShader);
    gl.glGetShaderiv(fragmentShader, GL_COMPILE_STATUS, status = malloc<Int>());
    _status = status.value;
    malloc.free(status);
    if (_status != GL_TRUE) throw Exception('An error occurred while compiling fragment shader ($fragmentShader):\n\n{GL.GetShaderInfoLog(fragmentShader)}');
    ...
    _status = status.value;
    malloc.free(status);
    if (_status != GL_TRUE) throw Exception('An error occured while linking program ($handle):\n\n{GL.GetProgramInfoLog(handle)}');
  ...
  static Future<Shader> load(String path) async {
    final sourceLines = await File(path).readAsLines();
